...
# controlled people vay by location
...
#...........................................................
## Antihypertensive therapy — Program-rate mode ----
#...........................................................

# This function reuses the ETIHAD machinery but substitutes exogenous
...
calculate_antihypertensive_impact_program <- function(intervention_rates,
                                                       ...
                                                       control_rate_vector,
                                                       drugcov = "p75") {
  cat(" - Calculating antihypertensive impact using PROGRAM control-rate vector\n")

  # ---- Steps 1–3: identical to the target-based function ----

  # Step 1: Baseline BP distribution (no treatment)
  bp_prob_base <- get.bp.prob(DT.in, rx = 0, drugaroc = "baseline")
  ...
  # Step 3: ETIHAD effect sizes (same as target-based)
  causes  <- c("ihd", "hhd", "istroke", "hstroke", "aod")
  bp_cats <- c("<120", "120-129", "130-139", "140-149",
               "150-159", "160-169", "170-179", "180+")
  ...
  # ---- Step 4: Exogenous coverage from control_rate_vector ----
  # control_rate_vector is a data.table with columns: year, control_rate
  # This REPLACES the linear scale-up logic entirely.

  dt_baseline <- merge(
    dt_baseline,
    control_rate_vector[, .(year, control_rate)],
    by = "year",
    all.x = TRUE
  )
  dt_baseline[is.na(control_rate), control_rate := 0]

  # Hypertensive BP bins eligible for treatment
  hypertensive_bins <- c("140-149", "150-159",
                         "160-169", "170-179", "180+")

  # Map exogenous control rate directly to coverage_t
  dt_baseline[, `:=`(coverage_0 = 0, coverage_t = 0)]
  dt_baseline[bp_cat %in% hypertensive_bins,
              coverage_t := pmin(pmax(control_rate, 0), 1)]

  # Non-hypertensive bins: no coverage
  dt_baseline[!bp_cat %in% hypertensive_bins,
              `:=`(coverage_t = 0, coverage_0 = 0)]

  dt_baseline[, control_rate := NULL]

  # ---- Steps 5–10: identical to the target-based function ----
  ...
  # Incremental coverage above baseline (coverage_0 = 0 here)
  dt_baseline[, coverage_delta := pmax(coverage_t - coverage_0, 0)]
  ...
  cat("  - Program control-rate vector applied successfully\n")
...
calculate_tfa_impact <- function(dt_tfa_scenarios,
  ...
  # For this assesment excersie tfa_target is vectorized and included in dt_tfa_scenarios, 
...
project.all <- function(Country, 
                        ...
                        baseline_statin_coverage = NULL,
                        # Program-rate mode parameters (NEW)
                        use_program_rates = FALSE,       # Switch to scenario-table mode
                        control_rate_vector = NULL,       # data.table(year, control_rate) — exogenous
                        dt_htn_enrollment = NULL           # data.table(location, year, age, sex, Nx_program, pop_program)
) {
  ...
  #...............................................................
  # Program-rate mode: substitute facility-level population
  # When use_program_rates = TRUE, replace national Nx with Nx_program
  # so that all absolute counts (sick, dead, well, pop) reflect the
  # program population. Rates (IR, CF) are unaffected.
  # Other interventions (sodium, TFA, statins) continue to use these
  # same counts — this is correct because in program-rate mode we are
  # modeling the program cohort, not the national population.
  
  if (use_program_rates && !is.null(dt_htn_enrollment)) {
    
    cat("  Substituting program population (Nx_program, pop_program)\n")
    
    # If Nx_program/pop_program already in base_rates, use them directly
    if (all(c("Nx_program", "pop_program") %in% names(intervention_rates))) {
      
      intervention_rates[!is.na(Nx_program), Nx := Nx_program]
      intervention_rates[!is.na(pop_program), pop := pop_program]
      intervention_rates[, c("Nx_program", "pop_program") := NULL]
      
      intervention_rates_bau[!is.na(Nx_program), Nx := Nx_program]
      intervention_rates_bau[!is.na(pop_program), pop := pop_program]
      intervention_rates_bau[, c("Nx_program", "pop_program") := NULL]
      
    } else {
      # Fallback: merge from dt_htn_enrollment
      dt_enroll_loc <- dt_htn_enrollment[location == Country]
      
      if (nrow(dt_enroll_loc) > 0) {
        intervention_rates <- merge(
          intervention_rates,
          unique(dt_enroll_loc[, .(location, year, age, sex, Nx_program, pop_program)]),
    ...
    cat("  Program population substituted for", Country, "\n")
  }
  ...
  if ("antihypertensive" %in% interventions) {
    ...
    if (use_program_rates && !is.null(control_rate_vector)) {
      # ---- PROGRAM-RATE MODE ----
      # Use exogenous control-rate vector instead of scale-up
      cat("  Mode: program-rate (exogenous control_rate_vector)\n")
      
      intervention_rates_drug <- calculate_antihypertensive_impact_program(
        intervention_rates_bau,
        Country,
        DT.in,
        dt_gbd_rr,
        control_rate_vector = control_rate_vector,
        drugcov             = drugcov
      )
    } else {
      # ---- TARGET-BASED MODE (original, unchanged) ----
      cat("  Mode: target-based scale-up\n")
...
run_multiple_scenarios <- function(Country,
                                   ...
                                   baseline_statin_coverage = NULL,
                                   # Program-rate mode parameters (NEW)
                                   use_program_rates = FALSE,
                                   control_rate_vector = NULL,
                                   dt_htn_enrollment = NULL) {
  ...
  for (scenario_name in names(scenario_list)) {
    ...
    results[[scenario_name]] <- project.all(
      ...
      # Program-rate mode (NEW — passed through to project.all)
      use_program_rates   = use_program_rates,
      control_rate_vector = control_rate_vector,
...
run_program_impact_scenarios <- function(Country,
  ...
  # Extract country-specific control-rate vectors
  dt_loc <- dt_htn_control_scenarios[location == Country]

  if (nrow(dt_loc) == 0) {
    cat("WARNING: No program-rate data for", Country, "— skipping.\n")
    return(NULL)
  }
...
# Scenarios: HTN and TFA scenarios will use the dynamic country-specific target if available, otherwise fall back to user-defined target_control
...
if (length(locs_program) > 0) {
  ...
  cat("\n=== Starting program-impact parallel runs for",
      length(locs_program), "countries ===\n")

  # Re-start cluster (if previously stopped) or reuse
  if (!exists("cl") || !inherits(tryCatch(clusterEvalQ(cl, 1),
                                           error = function(e) e), "list")) {
    cl <- makeCluster(ncores)
    registerDoParallel(cl)
  ...
  time_start_pgm <- Sys.time()

  results_program_list <- foreach(
    loc_idx        = seq_along(locs_program),
    .packages      = c("data.table", "dplyr"),
    .errorhandling = "pass",
    .verbose       = TRUE
  ) %dopar% {

    country <- locs_program[loc_idx]

    log_file <- file.path(
      wd_outp, "out_model",
      paste0("log_program_", country, ".txt")
    )
    sink(log_file, split = FALSE)

    cat("\n==============================\n")
    cat("Country (program-impact):", country, "\n")
    cat("Time:", as.character(Sys.time()), "\n")
    cat("==============================\n")
    ...
    res <- tryCatch({
      ...
      cat("ERROR in program-impact for", country, ":", e$message, "\n")
      return(NULL)
    })
  ...
  time_end_pgm <- Sys.time()
  cat("Program-impact runtime:",
      round(difftime(time_end_pgm, time_start_pgm, units = "mins"), 1),
      "minutes\n")

  successful_pgm <- sapply(results_program_list, function(x) !is.null(x))
  cat("\nProgram-impact successful:", sum(successful_pgm),
      "out of", length(locs_program), "\n")
  cat("Failed:", paste(locs_program[!successful_pgm], collapse = ", "), "\n")

  stopCluster(cl)

} else {
  cat("\nNo countries found in dt_htn_control_scenarios — skipping program-impact runs.\n")
}
